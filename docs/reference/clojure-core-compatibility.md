...
| `defstruct` | `defstruct` | Hara has no Clojure JVM record/class contract; portable immutable domain values use `defstruct` and protocols. |
...
| Clojure | Hara | Contract |
|---|---|---|
| `contains?` | `IFind/has?` | Protocol membership uses the Hara IFind/has? protocol method. |
