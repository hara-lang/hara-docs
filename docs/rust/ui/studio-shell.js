...
export function createStudioShell(root) {
  ...
  kernelActions.append(iconButton("＋", "Create kernel", "kernel/new"), iconButton("×", "Close kernel", "kernel/close"));
  ...
  const dispatch = (action, source) => shell.dispatchEvent(new CustomEvent("hara:studio-action", { bubbles: true, detail: { action, source } }));
  ...
  return {
    shell, main, projectButton: project, fileNewButton: fileNew, importButton: projectImport,
    consoleButton: consoleToggle, mobileOutputButton: mobileOutput, healthButton, statusPopover: popover,
    ...
    closeStatus() { popover.hidden = true; healthButton.setAttribute("aria-expanded", "false"); },
